...
-- BUG-16. A refusal is prose and belongs on the status line, not on a one-row button: written to
-- the button it spilled both ways across the inspector and landed on top of Inherit. So Core
-- answers both halves — the caption the control wears while a refusal stands, and the sentence the
-- line carries — and neither frame can drift from the refusal it describes.
...
H.ok(#caption <= 12, "…in a caption short enough for the control, which no reason ever is")
