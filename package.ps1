...
# every flavour's .toc. The flavour is chosen by the suffix on the file name — `Kitbag.toc` is
# Classic Era — which is why the name has to be Blizzard's vocabulary and not ours.
#
# Kitbag is a Classic Era addon and ships one flavour (2026-09-03). Mists, Cataclysm and Retail are
# gone from the repo entirely rather than held back: a .toc in the zip is a promise that the addon
# runs on that flavour, and none of those three had ever drawn a frame. $SHIPPED_TOCS stays a list
# because Tests/toc_test.lua reads the zip's contents out of it.
#
...
# on it — not when its .toc exists. That test is what emptied it: Retail was never launched and
# Cataclysm Classic no longer exists as a client, so neither could ever earn a place, and holding
# them back release after release was a cost with no end. Tests/toc_test.lua reads this list rather
# than restating it, and holds the README's "Supports …" line to the same set.
$SHIPPED_TOCS = @(
    "Kitbag.toc"        # Classic Era — the only flavour
)
